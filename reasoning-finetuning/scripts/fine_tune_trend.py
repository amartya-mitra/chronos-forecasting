#!/usr/bin/env python3
"""
Fine-tune chronos-t5-small on sarsim0-trend-test.arrow.
...
Outputs:
  checkpoints/trend-finetune/         fine-tuned model weights
  figures/finetune_trend/loss.png
  figures/finetune_trend/eval_fast_samples.png
  figures/finetune_trend/eval_trend_samples.png
"""
...
# ── paths ─────────────────────────────────────────────────────────────────────
DATA_PATH    = FINETUNING_ROOT / "data"  / "sarsim0-trend-test.arrow"
CKPT_DIR     = FINETUNING_ROOT / "checkpoints" / "trend-finetune"
FIGURES_DIR  = FINETUNING_ROOT / "figures" / "finetune_trend"
MODEL_ID     = "amazon/chronos-t5-small"

# ── hyperparameters ───────────────────────────────────────────────────────────
FAST_TOKEN_ID  = 4096
TREND_TOKEN_ID = 4097
EXTENDED_VOCAB = 4098          # vocab extended by 2 new mode tokens
...
LR            = 1e-4
GRAD_CLIP     = 1.0
EVAL_EVERY    = 250            # eval + log every N steps
EVAL_SAMPLES  = 20             # num_samples for stochastic decoding
EVAL_N_SHOW   = 5              # samples to plot per mode
EVAL_POOL_PER_MODE = 100       # held-out pool per mode
...
def load_dataset(path: Path) -> tuple[list[dict], list[dict]]:
    """
    Load Arrow file and return (train_rows, eval_rows).
    Holds out EVAL_POOL_PER_MODE samples per mode for evaluation.
    """
...
class CyclicLoader:
    ...
    def __init__(self, rows: list[dict], batch_size: int, seed: int = 0):
        ...
        self._pos       = len(rows)   # force reshuffle on first call
...
def tokenise_batch(
    ...
    """
    Returns (input_ids, attention_mask, labels) with mode prefix prepended.
    ...
    # Tokenise context (returns EOS-terminated ids for seq2seq)
    token_ids, attn_mask, scale = tokenizer.context_input_transform(contexts)

    # Prepend mode prefix token
    ...
    # Tokenise targets using same scale as context
...
def predict_with_mode(
    ...
    """Return median forecast (prediction_length,) using the mode-prefixed encoder."""
    ...
    raw = raw.reshape(1, num_samples, prediction_length)   # (1, S, T)

    preds = pipeline.tokenizer.output_transform(raw.cpu(), scale)  # (1, S, T)
    return preds[0].median(dim=0).values.numpy()           # (T,)
...
def evaluate(
    ...
    """Return (mean_mae_fast, mean_mae_trend) over the held-out eval pool."""
...
def plot_loss(
    ...
    # Smoothed line (moving average over 50 pts)
    ...
    fig.suptitle("Fine-tuning: chronos-t5-small  ·  trend-test dataset", fontweight="bold")
    plt.tight_layout()
...
def plot_eval_samples(
    ...
    fig.suptitle(
        f"Fine-tuned chronos-t5-small  ·  {mode}-mode evaluation  ·  {n} held-out samples\n"
...
def main():
    ...
    print("=" * 60)
    print("Fine-tuning: chronos-t5-small on trend-test dataset")
    ...
    inner = pipeline.model.model          # T5ForConditionalGeneration

    # Extend vocabulary to accommodate mode tokens (4096, 4097)
    ...
    if current_vocab < EXTENDED_VOCAB:
        # HuggingFace initialises new rows from the existing embedding distribution
    ...
    for step in range(1, N_STEPS + 1):
        ...
        if step % EVAL_EVERY == 0:
            pipeline.model.model = inner       # keep reference current
