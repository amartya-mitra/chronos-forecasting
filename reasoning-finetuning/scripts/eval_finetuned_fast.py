#!/usr/bin/env python3
"""
Evaluate the FAST-mode zero-shot forecasting of a joint-finetuned checkpoint.

Loads the finetuned model from:
  checkpoints/finetune-m4h-joint-{job_id}/
...
Outputs (saved into figures/finetune_m4h_joint_{job_id}/):
  eval_fast_samples.png   — sample context + prediction vs ground-truth
  eval_fast_metrics.png   — MAE / MASE histograms vs pretrained baseline
  eval_fast_summary.txt   — scalar metrics

Usage:
  python eval_finetuned_fast.py --job-id 46008
"""
...
def load_eval_rows(path: Path) -> list:
    """Load the same 100-row eval split used during joint training."""
...
def denorm_forecast(row: dict) -> np.ndarray:
    """Return ground-truth 48-step forecast in original units."""
...
def load_finetuned_pipeline(ckpt_dir: Path, device: torch.device) -> ChronosPipeline:
    """
    Load base ChronosPipeline (for tokenizer + config), then swap in the
    finetuned model weights from the saved checkpoint.
    """
...
def run_fast_inference(pipeline: ChronosPipeline, rows: list,
    ...
    """Run FAST-mode inference; return list of median 48-step forecasts (original units)."""
    ...
    for row in tqdm(rows, desc="FAST inference", unit="sample"):
        ...
        raw_bins = raw[:, 1:HORIZON + 1].reshape(1, NUM_SAMPLES, HORIZON)  # strip start token
        out = pipeline.tokenizer.output_transform(raw_bins.cpu(), scale)   # (1, S, 48)
...
def compute_metrics(rows: list, preds: list[np.ndarray]) -> dict:
    maes, mases = [], []
...
def plot_samples(rows: list, preds: list[np.ndarray], path: Path,
    ...
    fig.suptitle(
        f"FAST mode (token 4096)  ·  finetuned job {job_id}  ·  M4 Hourly eval pool\n"
    ...
    for ax, (key, label, colour, b_mean_k, b_med_k) in zip(axes, pairs):
        ...
        if baseline.get(b_mean_k):
            ax.axvline(baseline[b_mean_k], color="grey", lw=1.2, ls="-.",
                       label=f"Pretrained mean {baseline[b_mean_k]:.2f}")
        if baseline.get(b_med_k):
            ax.axvline(baseline[b_med_k], color="dimgrey", lw=1.0, ls=":",
                       label=f"Pretrained median {baseline[b_med_k]:.2f}")
...
def main():
    ...
    parser.add_argument("--job-id", required=True, help="Joint fine-tuning job ID")
    ...
    print("=" * 60)
    print(f"FAST-mode eval  ·  finetuned job {job_id}")
    ...
    summary_path.write_text(
        f"FAST-mode eval  ·  finetuned job {job_id}\n"
        f"n_eval      : {len(rows)}\n"
