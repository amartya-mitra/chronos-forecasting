#!/usr/bin/env python3
"""
preflight_m4daily.py — Pre-flight checks for M4 Daily sweep training.

Checks P1–P6 + P_new + P_daily on 3 smoke batches before submitting the training job:

  P1. Data loads correctly
      - num train/val series (3381/846), context length=93, future length=14
      - future = target[0:] confirmed (target_offset=0, no aux prefix)
      - values in O(100–10000) raw scale range

  P2. STL detects plausible period on 3/3 smoke batches
      - With 93-pt context, min_period=4, max_period=None (n//4=23)
      - Expect period=7 (weekly), or period=14 (biweekly)
      - Flag if period=1 or period=2 detected

  P3. Fix 1b trigger rate across 3 batches
      - Daily economic series may have trend-dominated segments (moderate trigger rate)
  ...
  P_daily. STL detects period=7 on at least 2/3 smoke batches.
           If missed, print detected periods and n//4 max_period value.

Config must match train_ddp.py --dataset m4daily --steps 2000:
  context_len=93, pred_len=14, target_offset=0
  batch_size=8, TOTAL_STEPS=2000, min_period=4, max_period=None (n//4=23), rs_threshold=0.4
"""
...
ARROW_PATH    = FINETUNING_ROOT / "data" / "m4daily-tsf.arrow"
CONTEXT_COL   = "context"
FORECAST_COL  = "target"
...
BATCH_SIZE    = 8
NUM_BATCHES   = 3
TOTAL_STEPS   = 2000    # M4 Daily training run length
MIN_PERIOD    = 4
...
print("=" * 70)
print("PREFLIGHT — M4 Daily sweep training")
...
max_period_effective = MAX_PERIOD  # explicitly set to 31 (93 // 3)
print(f"\n  max_period (effective): {max_period_effective}  (explicit)  "
      f"{'✓ period=7 detectable' if max_period_effective >= 7 else '✗ period=7 missed!'}")
...
# ══════════════════════════════════════════════════════════════════════════════
# P_daily — Period=7 detection check
# ══════════════════════════════════════════════════════════════════════════════

print("\n" + "=" * 70)
print("P_daily — DAILY PERIOD DETECTION")
print("  (M4 Daily economic series: dominant periods are 12-23 day monthly/biweekly cycles)")
print("  (Accept any period >= 7; MASE_PERIOD=7 is the eval metric, independent of STL)")
print("=" * 70)

# For M4 Daily economic series, period=7 is rarely the FFT-dominant period.
# Monthly business-day cycles (~20-22 days) and biweekly patterns (12-16 days)
# dominate instead. Accept any detected period >= 7 as plausible.
DAILY_MIN_PLAUSIBLE = 7
batches_plausible_daily = 0
for b_idx, periods_list in enumerate(all_periods):
    has_plausible = any(p >= DAILY_MIN_PLAUSIBLE for pds in periods_list for p in pds)
    n_plausible   = sum(1 for pds in periods_list if any(p >= DAILY_MIN_PLAUSIBLE for p in pds))
    if has_plausible:
        batches_plausible_daily += 1
    print(f"  Batch {b_idx}: plausible period (>={DAILY_MIN_PLAUSIBLE}) in {n_plausible}/{len(periods_list)} series  "
          f"{'✓' if has_plausible else '⚠'}")
    print(f"    All detected: {periods_list}")

p_daily_pass = batches_plausible_daily >= 2
print(f"\n  Batches with plausible period: {batches_plausible_daily}/{NUM_BATCHES}")
print(f"  max_period={MAX_PERIOD}  min_period={MIN_PERIOD}")
if not p_daily_pass:
    print(f"  ⚠  No meaningful periods detected — check STL configuration.")
print(f"  P_daily {'PASS  ✓' if p_daily_pass else 'FAIL ⚠  (no plausible period >= 7 in 2+/3 batches)'}")
...
print("\n" + "=" * 70)
print("P2 — STL PERIOD DETECTION  (M4 Daily: accept any period >= 7)")
print("  (Economic daily series have dominant 12-23 day cycles, not weekly 7-day)")
print("=" * 70)

batches_plausible = 0
for b_idx, periods_list in enumerate(all_periods):
    has_plausible = any(p >= 7 for pds in periods_list for p in pds)
    ...
    n_pl = sum(1 for pds in periods_list if any(p >= 7 for p in pds))
    print(f"  Batch {b_idx}: plausible period in {n_pl}/{len(periods_list)} series  {flag}")
    print(f"    All detected: {periods_list}")

p2_pass = batches_plausible >= 2
print(f"\n  Batches with plausible period (>= 7): {batches_plausible}/{NUM_BATCHES}")
print(f"  P2 {'PASS  ✓' if p2_pass else 'FAIL ⚠  (< 2/3 batches detected period >= 7)'}")
...
print(f"  (M4H reference: 0%;  M4 Monthly: ~1-2%;  M4 Daily expected: similar)")
...
print(f"  P_daily (weekly period):      {'PASS  ✓' if p_daily_pass else 'FAIL ⚠'}")

all_pass = p2_pass and p5_pass and p6_pass and p_new_pass and p_daily_pass
if all_pass:
...
assert p_daily_pass, "PREFLIGHT ABORT: weekly period not detected — STL may need max_period adjustment"
