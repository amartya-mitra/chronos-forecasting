#!/usr/bin/env python3
"""
Fine-tune chronos-t5-small on sarsim0-trend-test-v2.arrow.  (v5 — better mode-token init)

Dataset split: 70% fast-mode / 30% trend-mode.

Inherits decoder-side conditioning from v4:
  encoder          = [ctx_tokens | EOS]                       (513 tokens)
  decoder_input_ids = [mode_token | tgt_0 .. tgt_62]         (65 tokens)

Key change from v4: custom initialisation of the two new mode-token embeddings.

Problem with v4 (and earlier): HuggingFace resize_token_embeddings() initialises
new rows from a multivariate normal fitted to existing embeddings.  This places
the new tokens in the middle of the existing embedding cloud, making them hard
to distinguish from legitimate context/value tokens and giving them a weak
gradient signal.

Two initialisation strategies (select via EMBED_INIT_MODE):
  "eos_plus_noise"  — new_emb  = EOS_emb + N(0, σ)
      EOS (ID=1) was the pre-trained decoder_start_token_id, so the decoder
      already knows how to handle a token in that position.  Adding noise
      separates the two mode tokens from each other and from EOS while
      keeping them in a meaningful region of embedding space.

  "zero_plus_noise" — new_emb  = N(0, σ)
      Zero-mean with high σ pushes both tokens away from the existing
      embedding cluster, maximising expected cosine distance to all others.

σ = EMBED_INIT_STD_MULTIPLIER × (std of existing embedding matrix)
...
Usage:
  python fine_tune_trend_v5.py [--job-id JOB_ID]

Outputs (all under figures/finetune_trend_v5_{JOB_ID}/):
  config.txt  loss.png  eval_fast_samples.png  eval_trend_samples.png

Checkpoint: checkpoints/trend-finetune-v5-{JOB_ID}/
"""
...
# ── mode-token embedding initialisation ───────────────────────────────────────
# "eos_plus_noise" : new_emb = EOS_emb + N(0, σ)   ← recommended
#                   EOS (ID=1) was the pre-trained decoder_start_token_id.
# "zero_plus_noise": new_emb = N(0, σ)
#                   Zero-centred; maximises distance from existing cluster.
EMBED_INIT_MODE           = "eos_plus_noise"
EMBED_INIT_STD_MULTIPLIER = 2.0   # σ = multiplier × std(existing embeddings)
...
def init_mode_embeddings(inner, eos_token_id: int = 1) -> None:
    """
    Initialise the two newly added mode-token embeddings (IDs 4096, 4097).

    Strategy selected by EMBED_INIT_MODE:
      "eos_plus_noise"  — base = EOS embedding; new = base + N(0, σ)
      "zero_plus_noise" — base = 0;             new = N(0, σ)

    σ = EMBED_INIT_STD_MULTIPLIER × std(existing embedding matrix)

    Cosine similarities between the new tokens and EOS are printed so the
    initialisation quality can be verified in the log.
    """
    embed_weight = inner.get_input_embeddings().weight          # (vocab, d_model)
    existing_std = embed_weight[:FAST_TOKEN_ID].std().item()    # std of pre-trained rows
    noise_std    = EMBED_INIT_STD_MULTIPLIER * existing_std

    with torch.no_grad():
        if EMBED_INIT_MODE == "eos_plus_noise":
            base = embed_weight[eos_token_id].clone()
        else:  # "zero_plus_noise"
    ...
    # Report cosine similarities for sanity-check
...
def write_config(path: Path, job_id: str) -> None:
    ...
    lines = [
        ...
        f"Seed            : {SEED}",
        "",
        "Architecture (same as v4) — decoder-side mode conditioning:",
        "  encoder          = [ctx_tokens | EOS]               (513 tokens)",
        "  decoder_input_ids = [mode_token | tgt_0..tgt_62]   (65 tokens)",
        "",
        f"Key change vs v4 — mode-token embedding initialisation:",
        f"  EMBED_INIT_MODE           : {EMBED_INIT_MODE}",
        f"  EMBED_INIT_STD_MULTIPLIER : {EMBED_INIT_STD_MULTIPLIER}",
        "  'eos_plus_noise'  → new_emb = EOS_emb + N(0, σ)",
        "  'zero_plus_noise' → new_emb = N(0, σ)",
        "  σ = multiplier × std(existing embedding matrix)",
    ]
    path.write_text("\n".join(lines) + "\n")
...
def plot_loss(
    ...
    fig.suptitle(
        f"Fine-tuning v5: chronos-t5-small  ·  70% fast / 30% trend  ·  decoder-mode + embed init  ·  job {job_id}",
        fontweight="bold",
    )
    plt.tight_layout()
    path.parent.mkdir(parents=True, exist_ok=True)
...
def plot_eval_samples(
    ...
    fig.suptitle(
        f"Fine-tuned chronos-t5-small (v5)  ·  {mode}-mode eval  ·  job {job_id}\n"
...
def main():
    ...
    FIGURES_DIR = FINETUNING_ROOT / "figures"     / f"finetune_trend_v5_{job_id}"
    CKPT_DIR    = FINETUNING_ROOT / "checkpoints" / f"trend-finetune-v5-{job_id}"

    set_seed(SEED)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("=" * 60)
    print("Fine-tuning v5: chronos-t5-small  |  trend-test-v2  |  decoder-mode + better embed init")
