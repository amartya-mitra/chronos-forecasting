#!/usr/bin/env python3
"""
preflight_m4weekly.py — Pre-flight checks for M4 Weekly sweep training.

Checks P1–P6 + P_new + P_weekly on 3 smoke batches before submitting training:

  P1. Data loads correctly
      - num train/val series, context=104, future=13
      - target_offset=0 confirmed
      - values in O(100–100000) raw scale range

  P2. STL detects plausible period on 2/3 smoke batches
      - With 104-pt context, min_period=4, max_period=104 (n_cycles=1)
      - Accept any period >= 26 (half-year cycle or longer)
      - Flag cycles_available = 104 / 52 = 2.0
  ...
  P_weekly. STL detects plausible annual/seasonal period (>= 26) on 2/3 batches.
            Report cycles_available = CONTEXT_LEN / 52 prominently.

Config must match train_ddp.py --dataset m4weekly --steps 500:
  context_len=104, pred_len=13, target_offset=0
  batch_size=4, TOTAL_STEPS=500, min_period=4, max_period=104, rs_threshold=0.4
"""
...
ARROW_PATH    = FINETUNING_ROOT / "data" / "m4weekly-tsf.arrow"
CONTEXT_COL   = "context"
FORECAST_COL  = "target"
TARGET_OFFSET = 0       # future = target[:] — raw future only, no aux prefix
CONTEXT_LEN   = 80      # all 359 series; cycles=1.54 (< 2 — flagged)
PRED_LEN      = 13      # M4 weekly forecast horizon
TRAIN_SEED    = 42
BATCH_SIZE    = 4       # small dataset, batch_size=4 per GPU
NUM_BATCHES   = 3
TOTAL_STEPS   = 500     # M4 Weekly training run length
MIN_PERIOD    = 4
...
print("=" * 70)
print("PREFLIGHT — M4 Weekly sweep training")
print(f"Device: {DEVICE}")
print("=" * 70)

# Cycles available — critical check
cycles_avail = CONTEXT_LEN / 52.0
print(f"\n  ╔══════════════════════════════════════════════════════╗")
print(f"  ║  CYCLES AVAILABLE = {cycles_avail:.2f}  (context={CONTEXT_LEN} / period=52)  ║")
if cycles_avail < 2.0:
    print(f"  ║  ⚠  < 2 complete cycles — STL borderline for period=52  ║")
    print(f"  ║     n_cycles=1: max_period = {MAX_PERIOD}                   ║")
else:
...
if 1.0 < ctx_absmax < 1e9:
    print(f"  Scale check:  |ctx|_max = {ctx_absmax:.1f}  ✓  (raw scale)")
else:
    print(f"  Scale check:  |ctx|_max = {ctx_absmax:.1f}  ⚠  (unexpected scale)")

print(f"\n  max_period={MAX_PERIOD}  (n_cycles=1, explicit)  "
      f"{'✓ period=52 detectable' if MAX_PERIOD >= 52 else '✗ period=52 missed!'}")
print(f"  cycles_available = {CONTEXT_LEN}/{52} = {cycles_avail:.2f}  "
      f"{'✓ >= 2.0' if cycles_avail >= 2.0 else '⚠ < 2.0 — STL borderline'}")
...
c500 = min(0.5 + 0.3 * (TOTAL_STEPS / TOTAL_STEPS), 0.8)
assert abs(c0   - 0.50) < 1e-6, f"P4 FAIL: step-0 ceiling={c0:.6f}, expected 0.50"
assert abs(c500 - 0.80) < 1e-6, f"P4 FAIL: step-500 ceiling={c500:.6f}, expected 0.80"
print(f"\n  P4 PASS  (step=0 → 0.50, step=500 → 0.80)\n")
...
# ══════════════════════════════════════════════════════════════════════════════
# P_weekly — Annual period detection check
# ══════════════════════════════════════════════════════════════════════════════

print("\n" + "=" * 70)
print("P_weekly — WEEKLY PERIOD DETECTION")
print(f"  context_len={CONTEXT_LEN}, max_period={MAX_PERIOD} (n_cycles=1)")
print(f"  cycles_available = {cycles_avail:.2f}  (context={CONTEXT_LEN} / period=52)")
print(f"  Accepting any period >= 26 (half-year or annual cycle)")
print("=" * 70)

WEEKLY_MIN_PLAUSIBLE = 26  # half-year cycle or longer
batches_plausible_weekly = 0

for b_idx, periods_list in enumerate(all_periods):
    has_plausible = any(p >= WEEKLY_MIN_PLAUSIBLE for pds in periods_list for p in pds)
    has_52        = any(p == 52 for pds in periods_list for p in pds)
    n_plausible   = sum(1 for pds in periods_list if any(p >= WEEKLY_MIN_PLAUSIBLE for p in pds))
    if has_plausible:
        batches_plausible_weekly += 1
    print(f"  Batch {b_idx}: plausible period (>={WEEKLY_MIN_PLAUSIBLE}) in "
          f"{n_plausible}/{len(periods_list)} series  "
          f"{'✓' if has_plausible else '⚠'}  "
          f"{'(period=52 detected)' if has_52 else ''}")
    print(f"    All detected: {periods_list}")

p_weekly_pass = batches_plausible_weekly >= 2
print(f"\n  Batches with plausible period: {batches_plausible_weekly}/{NUM_BATCHES}")
print(f"  max_period={MAX_PERIOD}  min_period={MIN_PERIOD}")
if not p_weekly_pass:
    print(f"  ⚠  No plausible annual/seasonal period detected — check STL config")
    print(f"     Verify that max_period={MAX_PERIOD} >= 52 (current: {'YES' if MAX_PERIOD >= 52 else 'NO'})")
print(f"  P_weekly {'PASS  ✓' if p_weekly_pass else 'FAIL ⚠  (no period >= 26 in 2+/3 batches)'}")
...
print("\n" + "=" * 70)
print("P2 — STL PERIOD DETECTION  (M4 Weekly: accept any period >= 26)")
print("  (Annual economic cycles: period=52 typical; accept half-year >= 26)")
print("=" * 70)

batches_plausible = 0
for b_idx, periods_list in enumerate(all_periods):
    has_plausible = any(p >= 26 for pds in periods_list for p in pds)
    ...
    n_pl = sum(1 for pds in periods_list if any(p >= 26 for p in pds))
    print(f"  Batch {b_idx}: plausible period in {n_pl}/{len(periods_list)} series  {flag}")
    print(f"    All detected: {periods_list}")

p2_pass = batches_plausible >= 2
print(f"\n  Batches with plausible period (>= 26): {batches_plausible}/{NUM_BATCHES}")
print(f"  P2 {'PASS  ✓' if p2_pass else 'FAIL ⚠  (< 2/3 batches detected period >= 26)'}")
...
print(f"  (M4 Daily reference: ~2.5%)")
...
print(f"  P_weekly (plausible period):    {'PASS  ✓' if p_weekly_pass else 'FAIL ⚠'}")
print(f"\n  cycles_available = {cycles_avail:.2f}  "
      f"({'>= 2.0: reliable' if cycles_avail >= 2.0 else '< 2.0: borderline, n_cycles=1 used'})")

all_pass = p2_pass and p5_pass and p6_pass and p_new_pass and p_weekly_pass
if all_pass:
...
assert p_weekly_pass, "PREFLIGHT ABORT: no plausible period >= 26 in 2+/3 batches — check STL config"
