#!/usr/bin/env python3
"""
Joint TSF + FAST fine-tuning on M4 Hourly data.

For each training step a batch is drawn randomly from either:
  • TSF  mode (token 4102): Trend(48) | SEP_1 | Seasonal(48) | SEP_2 | Forecast(48)  → gen_len=146
  • FAST mode (token 4096): direct 48-step forecast, no SEPs                          → gen_len=48
...
Usage
-----
  torchrun --nproc_per_node=4 --master_port=29501 fine_tune_m4h_joint.py --job-id $SLURM_JOB_ID
  torchrun --nproc_per_node=4 --master_port=29501 fine_tune_m4h_joint.py --job-id $SLURM_JOB_ID --fast-ratio 0.3
"""
...
BASELINE_REF = FINETUNING_ROOT / "figures" / "evals" / "eval_pretrained_m4hourly" / "baseline_mae.txt"
MODEL_ID     = "amazon/chronos-t5-small"

# ── M4 Hourly dimensions ───────────────────────────────────────────────────────
CONTEXT_LEN = 96
HORIZON     = 48   # segment length for both modes
...
FAST_CFG = {
    ...
    "segment_len": HORIZON,
    "ctx_len":    CONTEXT_LEN,
    "pred_len":   HORIZON,       # 48 direct forecast tokens
...
def write_config(path: Path, job_id: str, fast_ratio: float) -> None:
    ...
    path.write_text("\n".join([
        ...
        f"Job ID      : {job_id}",
        f"Script      : fine_tune_m4h_joint.py",
        f"Model       : {MODEL_ID}",
        f"Dataset     : m4hourly-tsf.arrow",
        ...
        f"TSF pred    : 144 tokens (3×48) + 2 SEPs → gen_len=146",
        f"FAST pred   : 48 tokens  (direct forecast, no SEPs)",
...
def pretokenise_tsf(rows: list, tokenizer) -> list:
    ...
    Tokenise rows for TSF mode.
      labels        = [trend(48) | SEP_1 | seasonal(48) | SEP_2 | forecast(48) | EOS]
      dec_input_ids = [4102 | trend(48) | SEP_1 | seasonal(48) | SEP_2 | forecast(48)]
    """
    N    = len(rows)
    seg  = HORIZON
...
def pretokenise_fast(rows: list, tokenizer) -> list:
    ...
    Tokenise rows for FAST mode.
      target        = forecast_norm only  (last 48 of the 144-token target)
      labels        = [forecast(48) | EOS]
      dec_input_ids = [4096 | forecast(48)]
    """
    N = len(rows)
    ...
    return [{
        "item_id":       rows[i]["item_id"],
        "context":       rows[i]["context"],
        "target":        rows[i]["target"],    # full 144-token target kept for TSF eval reference
        "s_parts":       rows[i]["s_parts"],
        "input_ids":     token_ids[i],
        "attn_mask":     attn_mask[i],
        "dec_input_ids": dec_input_ids[i],
...
def predict_tsf(pipeline, contexts: list, device) -> np.ndarray:
    """Generate TSF predictions; return (B, 144) normalised values."""
    ...
    # Strip SEPs: [part0(48) | SEP_1(1) | part1(48) | SEP_2(1) | part2(48)]
...
def predict_fast(pipeline, contexts: list, device) -> np.ndarray:
    """Generate FAST predictions; return (B, 48) in original units."""
    model_cfg = pipeline.model.config
...
def plot_loss(loss_steps, losses, eval_steps, tsf_maes, fast_maes,
    ...
    # FAST MAE vs pretrained baseline
    ...
    fig.suptitle(
        f"Joint TSF+FAST  ·  chronos-t5-small  ·  job {job_id}"
...
def plot_eval_samples_tsf(pipeline, eval_rows: list, device, path: Path,
    ...
    fig.suptitle(
        f"Joint TSF_M4H  ·  job {job_id}  ·  fast_ratio={fast_ratio:.0%}\n"
...
def main():
    ...
    parser.add_argument("--fast-ratio", type=float, default=0.20,
                        help="Fraction of batches using FAST mode (default 0.20)")
    args       = parser.parse_args()
    job_id     = args.job_id
    fast_ratio = args.fast_ratio

    FIGURES_DIR = FINETUNING_ROOT / "figures" / "finetunes" / f"finetune_m4h_joint_{job_id}"
    ...
    if IS_MAIN:
        ...
        print("=" * 60)
        print("Joint TSF+FAST fine-tuning: chronos-t5-small  (M4 Hourly)")
    ...
    tsf_loader  = CyclicLoader(tsf_shard,  per_rank_batch,
                               seed=SEED       + (LOCAL_RANK if IS_DDP else 0))
    fast_loader = CyclicLoader(fast_shard, per_rank_batch,
                               seed=SEED+1000  + (LOCAL_RANK if IS_DDP else 0))
    ...
    for step in pbar:
        ...
        if step % EVAL_EVERY == 0:
            ...
            if IS_MAIN:
                ...
                # Early stopping driven by TSF MAE (primary objective)
