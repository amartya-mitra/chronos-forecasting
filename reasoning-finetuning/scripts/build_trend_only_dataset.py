#!/usr/bin/env python3
"""
Build sarsim0-trend-only.arrow from sarsim0-10k.arrow.

Uses only the 6,000 "reasoning" rows (fast rows have empty trend/seasonal).
Every row receives the "trend" mode tag.
  context (512 steps) → target = trend component (64 future steps)

Outputs:
  data/sarsim0-trend-only.arrow
  figures/dataset_trend_only/samples.png   (5 sample plots)
"""
...
def load_source(path: Path) -> list[dict]:
    ...
    rows = []
    for item_id, mode, ctx, trend in zip(
        ...
        if mode != "reasoning":
            continue   # fast rows have empty trend
        rows.append({
            "item_id": item_id,
            "context": np.array(ctx,   dtype=np.float32),
            "target":  np.array(trend, dtype=np.float32),
        })
    return rows
...
def plot_samples(dataset: list[dict], path: Path, n: int = 5) -> None:
    ...
    fig.suptitle(
        "Trend-Only Dataset  ·  Sample Plots\n"
        "[trend-mode prefix  |  input context  →  target trend component]",
    ...
    for idx, pick in enumerate(picks):
        ...
        ax.plot(tgt_x, d["target"],  color=_TREND_COL, lw=1.6, label="Target trend")
...
def main() -> None:
    print("=" * 60)
    print("Building SarSim0 Trend-Only Dataset")
