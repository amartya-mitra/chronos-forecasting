#!/usr/bin/env python3
"""
Build sarsim0-stf.arrow from sarsim0-10k.arrow.

Uses only the 6,000 "reasoning" rows (which carry seasonal, trend, and forecast).
Mode tag: "STF".
  context (512) → target = [seasonal(64) | trend(64) | forecast(64)]  (192 steps total)

Per-component scale normalisation
----------------------------------
Each 64-step segment is rescaled so that its magnitude matches the context scale
(mean |context|) before concatenation.  After the tokenizer divides by context
scale all three segments are O(1) in bin-space regardless of their natural
amplitude ratios.

Stored per-row scale factors (original, before rescaling):
  s_first  — mean |seasonal|
  s_second — mean |trend|
  s_third  — mean |forecast|

Used in the training script to invert the normalisation for eval plots.

Outputs:
  data/sarsim0-stf.arrow
  figures/dataset_stf/samples.png   (5 sample plots)
"""
...
OUTPUT_PATH = FINETUNING_ROOT / "data" / "sarsim0-stf.arrow"
FIGURES_DIR = FINETUNING_ROOT / "figures" / "datasets" / "dataset_stf"

CONTEXT_LENGTH    = 512
PREDICTION_LENGTH = 192   # 64 seasonal + 64 trend + 64 forecast
SCALE_EPS         = 1e-6
...
def build_dataset(rows: list[dict]) -> list[dict]:
    ...
    for r in rows:
        ...
        out.append({
            ...
            "mode":     "STF",
            "context":  r["context"],
            # STF order: [seasonal | trend | forecast], all at context-scale magnitude
            "target":   np.concatenate([seasonal_norm, trend_norm, forecast_norm]),   # (192,)
            "s_first":  s_seasonal,
            "s_second": s_trend,
            "s_third":  s_forecast,
        })
...
def plot_samples(dataset: list[dict], path: Path, n: int = 5) -> None:
    ...
    fig.suptitle(
        "STF Dataset  ·  Sample Plots  (targets rescaled to context-scale magnitude)\n"
        "[STF-prefix  |  context(512)  →  seasonal_norm(64) | trend_norm(64) | forecast_norm(64)]",
...
def main() -> None:
    print("=" * 60)
    print("Building SarSim0 STF Dataset  (seasonal | trend | forecast, 192 steps)")
