#!/usr/bin/env python3
"""
Fine-tune chronos-t5-small on sarsim0-trend-only.arrow.

All 6,000 rows use the TREND mode token (4097).
  encoder           = [ctx_tokens | EOS]               (513 tokens)
  decoder_input_ids = [TREND_TOKEN | tgt_0 .. tgt_62]  (65 tokens)

Embedding init: EOS + noise (same as v5).

Outputs (figures/finetune_trend_only_{JOB_ID}/):
  config.txt  loss.png  eval_samples.png

Checkpoint: checkpoints/finetune-trend-only-{JOB_ID}/
"""
...
# ── constants ─────────────────────────────────────────────────────────────────
TREND_TOKEN_ID = 4097
EXTENDED_VOCAB = 4098   # adds tokens 4096 (fast) and 4097 (trend); only 4097 is used here
...
DATA_PATH = FINETUNING_ROOT / "data" / "sarsim0-trend-only.arrow"
...
def init_mode_embeddings(inner) -> None:
    ...
    existing_std = w[:TREND_TOKEN_ID].std().item()
    noise_std    = EMBED_INIT_STD_MULTIPLIER * existing_std
    with torch.no_grad():
        base = w[1].clone()   # EOS token (ID=1)
        gen  = torch.Generator(); gen.manual_seed(SEED)
        # init both slots so resize doesn't leave 4096 random
        for tok_id in (4096, TREND_TOKEN_ID):
            w[tok_id] = base + noise_std * torch.randn(w.shape[1], generator=gen, dtype=w.dtype)
    t_emb = w[TREND_TOKEN_ID].float()
    e_emb = w[1].float()
    cos = F.cosine_similarity(t_emb.unsqueeze(0), e_emb.unsqueeze(0)).item()
    print(f"  Embed init : eos_plus_noise,  noise_std={noise_std:.5f}")
    print(f"  Cosine sim : trend↔EOS={cos:.3f}")
...
def write_config(path: Path, job_id: str) -> None:
    ...
    path.write_text("\n".join([
        ...
        f"Job ID   : {job_id}",
        f"Script   : fine_tune_trend_only.py",
        f"Model    : {MODEL_ID}",
        f"Dataset  : sarsim0-trend-only.arrow  (6k rows, all trend-mode)",
        f"Token ID : TREND_TOKEN_ID={TREND_TOKEN_ID}",
...
def pretokenise(rows, tokenizer):
    ...
    trend_col     = torch.full((N, 1), TREND_TOKEN_ID, dtype=token_ids.dtype)
    dec_input_ids = torch.cat([trend_col, label_ids[:, :-1]], dim=1)   # (N, 65)
    out = []
...
def predict_batch(pipeline, contexts, device):
    ...
    raw = pipeline.model.model.generate(
        ...
        generation_config=GenerationConfig(
            ...
            decoder_start_token_id=TREND_TOKEN_ID,
...
def plot_loss(loss_steps, losses, eval_steps, maes, path, job_id):
    ...
    axes[1].plot(eval_steps, maes, color=_TREND_COL, lw=1.8, marker="s", ms=3, label="Trend MAE")
    axes[1].set_title("Eval MAE — Trend Mode", fontweight="bold")
    axes[1].set_xlabel("Step"); axes[1].legend(fontsize=8)
    axes[1].spines[["top","right"]].set_visible(False)
    fig.suptitle(f"Fine-tuning trend-only: chronos-t5-small  ·  job {job_id}", fontweight="bold")
    plt.tight_layout()
    path.parent.mkdir(parents=True, exist_ok=True)
...
def plot_eval_samples(pipeline, eval_rows, device, path, job_id, n=EVAL_N_SHOW):
    ...
    fig.suptitle(
        f"fine_tune_trend_only (v-trend)  ·  job {job_id}\n"
        "seagreen = target trend   crimson = model prediction",
    ...
    for idx, (row, pred) in enumerate(zip(sel, preds)):
        ...
        ax.plot(tgt_x, row["target"],  color=_TREND_COL, lw=1.8, label="Target trend")
...
def main():
    ...
    FIGURES_DIR = FINETUNING_ROOT / "figures" / "finetunes" / f"finetune_trend_only_{job_id}"
    CKPT_DIR    = FINETUNING_ROOT / "checkpoints" / f"finetune-trend-only-{job_id}"

    set_seed(SEED)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    print("=" * 60)
    print("Fine-tuning trend-only: chronos-t5-small")
    print(f"  Job ID    : {job_id}")
    print(f"  Device    : {device}")
    print(f"  Steps     : {N_STEPS:,}  |  batch={BATCH_SIZE}  |  LR={LR}")
    print(f"  Token ID  : TREND={TREND_TOKEN_ID}")
    ...
    for step in pbar:
        ...
        if step % EVAL_EVERY == 0:
            ...
            tqdm.write(f"  [eval] step={step:,}  trend MAE={mae:.4f}")
            inner.train()
    ...
    print(f"Final trend MAE : {maes[-1]:.4f}")
