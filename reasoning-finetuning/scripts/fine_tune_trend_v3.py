#!/usr/bin/env python3
"""
Fine-tune chronos-t5-small on sarsim0-trend-test-v2.arrow.  (v3 — optimised)

Dataset split: 70% fast-mode / 30% trend-mode.

Key optimisations over v2:
  - Batch size 128 (was 32) — fills H100 VRAM better
  - Training data pre-tokenised once on CPU; full input_ids (with mode prefix)
    and attn_mask are pre-concatenated, so each training step is just
    torch.stack + one .to(device) per tensor — no per-step CPU work.
  - Eval inference is batched (EVAL_BATCH=32) instead of one-at-a-time,
    so each eval checkpoint is ~7 GPU calls instead of 200.
  - Eval every 500 steps (was 250) — halves eval overhead.
  - tqdm progress bar for both training loop and eval batches.

Two prefix tokens distinguish the task:
  4096  (<fast_mode>)  → predict 64-step future forecast
  4097  (<trend_mode>) → predict 64-step trend of the input context

Usage:
  python fine_tune_trend_v3.py [--job-id JOB_ID]
...
def write_config(path: Path, job_id: str) -> None:
    ...
    lines = [
        ...
        "",
        "Optimisations vs v2:",
        "  - Batch 32 → 128 (better H100 VRAM utilisation)",
        "  - Pre-tokenised + pre-concatenated input_ids (no per-step CPU work)",
        "  - Batched eval inference (EVAL_BATCH=32, was 1)",
        "  - Eval frequency 250 → 500 steps",
        "  - tqdm progress bars",
        "",
        "Motivation: prior run (70% trend / 30% fast) showed the model",
        "predicting trend even in fast-mode. Rebalancing to 70% fast / 30%",
        "trend to bias the model toward retaining base forecast behaviour.",
    ]
    path.write_text("\n".join(lines) + "\n")
...
def pretokenise(rows: list[dict], tokenizer) -> list[dict]:
    ...
    Stores the fully assembled encoder input (mode prefix already prepended):
      input_ids : (514,) int tensor  [mode_token | context_tokens | EOS]
      attn_mask : (514,) bool tensor
      labels    : (65,)  int tensor  [-100 for padding]

    No per-step cat or reshape needed during training — assemble_batch just
    stacks these tensors and moves them to device.
    """
    N = len(rows)
    ...
    # Tokenise context and targets on CPU
    ...
    # Build mode prefix column and pre-concatenate — eliminates per-step cat
    ...
    for i, d in enumerate(rows):
        tokenised.append({
            ...
            "input_ids": full_input_ids[i],   # (514,) CPU tensor
            "attn_mask": full_attn_mask[i],   # (514,) CPU tensor
...
def assemble_batch(
    ...
    """
    input_ids = torch.stack([d["input_ids"] for d in batch]).to(device)  # (B, 514)
    attn_mask = torch.stack([d["attn_mask"] for d in batch]).to(device)  # (B, 514)
...
def predict_batch_with_mode(
    ...
    mode_prefix = torch.full((B, 1), mode_token_id, dtype=token_ids.dtype)
...
def plot_loss(
    ...
    fig.suptitle(
        f"Fine-tuning v3: chronos-t5-small  ·  70% fast / 30% trend  ·  job {job_id}",
        fontweight="bold",
    )
    plt.tight_layout()
    path.parent.mkdir(parents=True, exist_ok=True)
...
def plot_eval_samples(
    ...
    fig.suptitle(
        f"Fine-tuned chronos-t5-small (v3)  ·  {mode}-mode eval  ·  job {job_id}\n"
...
def main():
    ...
    print("=" * 60)
    print("Fine-tuning v3: chronos-t5-small  |  trend-test-v2")
