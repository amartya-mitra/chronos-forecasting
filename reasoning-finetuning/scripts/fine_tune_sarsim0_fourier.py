#!/usr/bin/env python3
"""
Fourier-component fine-tuning on SarSim0 data.

The model is trained to predict the top-3 Fourier components of the forecast
window, given the input context.  This is an analogue of the TSF experiments
where the model predicted Trend | SEP | Seasonal | SEP | Forecast — here it
predicts Fourier-1 | SEP | Fourier-2 | SEP | Fourier-3 with forced SEP
injection at fixed decode positions (same mechanism as TSF).

Mode token : 4105  (FOURIER — extends vocab to 4106)
Output     : Fourier-1(64) | SEP_1 | Fourier-2(64) | SEP_2 | Fourier-3(64)
             → 192 value tokens + 2 SEPs = gen_len 194

Evaluation (every EVAL_EVERY steps, rank 0 only)
  Fourier MAE : normalised MAE vs stored 3-component target  (early-stop metric)

Usage (single GPU):
  python fine_tune_sarsim0_fourier.py --job-id $SLURM_JOB_ID

Usage (multi-GPU via torchrun):
  torchrun --standalone --nproc_per_node=4 fine_tune_sarsim0_fourier.py --job-id $SLURM_JOB_ID
"""
...
# ── paths ─────────────────────────────────────────────────────────────────────
DATA_PATH = FINETUNING_ROOT / "data" / "sarsim0-fourier.arrow"
MODEL_ID  = "amazon/chronos-t5-small"

# ── dimensions ────────────────────────────────────────────────────────────────
CONTEXT_LEN = 512
HORIZON     = 64    # length of each Fourier component segment
N_PARTS     = 3

# ── special tokens ────────────────────────────────────────────────────────────
FOURIER_TOKEN_ID = 4105
SEP_1_TOKEN_ID   = 4103
SEP_2_TOKEN_ID   = 4104
EXTENDED_VOCAB   = 4106   # resize embeddings to this size

# ── Fourier mode config ───────────────────────────────────────────────────────
FOURIER_CFG = {
    "token_id":   FOURIER_TOKEN_ID,
    "n_parts":    N_PARTS,
    "segment_len": HORIZON,
    "pred_len":   N_PARTS * HORIZON,             # 192
    "gen_len":    N_PARTS * HORIZON + 2,         # 194  (192 + 2 SEPs)
    # SEP_1 forced after first 64 tokens; SEP_2 after next 64 (positions are
    # 1-indexed and include the leading mode token)
    "forced":     [(HORIZON + 1, SEP_1_TOKEN_ID),
                   (2 * HORIZON + 2, SEP_2_TOKEN_ID)],
    "part_names": ["Fourier-1 (dominant)", "Fourier-2", "Fourier-3"],
    "part_cols":  ["crimson", "darkorange", "seagreen"],
}
...
def init_mode_embeddings(inner) -> None:
    """Initialise all new tokens (4096–4105) as EOS + noise."""
    ...
    cos = F.cosine_similarity(
        w[FOURIER_TOKEN_ID].float().unsqueeze(0), w[1].float().unsqueeze(0)
    ).item()
    print(f"  Cosine sim : FOURIER↔EOS = {cos:.3f}")
...
def write_config(path: Path, job_id: str) -> None:
    ...
    path.write_text("\n".join([
        ...
        f"Job ID      : {job_id}",
        f"Script      : fine_tune_sarsim0_fourier.py",
        f"Model       : {MODEL_ID}",
        f"Dataset     : sarsim0-fourier.arrow",
        f"FOURIER tok : {FOURIER_TOKEN_ID}  |  SEP_1={SEP_1_TOKEN_ID}  SEP_2={SEP_2_TOKEN_ID}",
        f"Vocab       : extended to {EXTENDED_VOCAB}",
        f"Steps       : {N_STEPS:,}  |  batch={BATCH_SIZE}  |  LR={LR}",
        f"Ctx len     : {CONTEXT_LEN}  |  Horizon : {HORIZON}  |  Parts : {N_PARTS}",
        f"Output      : {N_PARTS}×{HORIZON} tokens + 2 SEPs → gen_len={FOURIER_CFG['gen_len']}",
        f"Early stop  : patience={EARLY_STOP_PATIENCE}  min_delta={EARLY_STOP_MIN_DELTA}",
        "",
        "Objective: predict top-3 forward-looking Fourier components from context.",
...
def pretokenise(rows: list, tokenizer) -> list:
    """
    labels        = [F1(64) | SEP_1 | F2(64) | SEP_2 | F3(64) | EOS]
    dec_input_ids = [FOURIER | F1(64) | SEP_1 | F2(64) | SEP_2 | F3(64)]
    """
    N   = len(rows)
    seg = HORIZON
    ...
    parts    = [label_ids[:, i*seg:(i+1)*seg] for i in range(N_PARTS)]
    ...
    labels        = labels_seq.masked_fill(labels_seq < 0, -100)
    mode_col      = torch.full((N, 1), FOURIER_TOKEN_ID, dtype=token_ids.dtype)
...
@torch.no_grad()
def predict_fourier(pipeline, contexts: list, device) -> np.ndarray:
    """Generate Fourier predictions; return (B, 192) normalised values."""
    seg       = HORIZON
    gen_len   = FOURIER_CFG["gen_len"]
    ...
    lp  = LogitsProcessorList([SEPForcer(FOURIER_CFG["forced"])])
    raw = pipeline.model.model.generate(
        ...
        generation_config=GenerationConfig(
            ...
            decoder_start_token_id=FOURIER_TOKEN_ID,
    ...
    B   = len(contexts)
    raw = raw[:, 1:]   # strip leading mode token
    # strip SEPs: [F1(64) | SEP_1 | F2(64) | SEP_2 | F3(64)]
    raw_bins = torch.cat([raw[:, :seg], raw[:, seg+1:2*seg+1], raw[:, 2*seg+2:]], dim=1)
    raw_bins = raw_bins.reshape(B, EVAL_SAMPLES, FOURIER_CFG["pred_len"])
    return pipeline.tokenizer.output_transform(raw_bins.cpu(), scale).median(dim=1).values.numpy()


# ── evaluation ────────────────────────────────────────────────────────────────

def evaluate(pipeline, eval_rows: list, device) -> float:
    pipeline.model.model.eval()
    maes = []
    for i in range(0, len(eval_rows), EVAL_BATCH):
        batch = eval_rows[i:i+EVAL_BATCH]
        preds = predict_fourier(pipeline, [r["context"] for r in batch], device)
...
def plot_loss(loss_steps, losses, eval_steps, fourier_maes, path: Path, job_id: str) -> None:
    ...
    axes[1].plot(eval_steps, fourier_maes, color="darkorange", lw=1.8, marker="o", ms=3,
                 label="Fourier MAE (normalised)")
    axes[1].set_title("Eval MAE — Fourier Mode", fontweight="bold")
    axes[1].set_xlabel("Step"); axes[1].legend(fontsize=8)
    axes[1].spines[["top", "right"]].set_visible(False)

    fig.suptitle(
        f"Fourier Fine-tune  ·  chronos-t5-small  ·  SarSim0  ·  job {job_id}",
        fontweight="bold",
    )
    plt.tight_layout()
    path.parent.mkdir(parents=True, exist_ok=True)
...
def plot_eval_samples(pipeline, eval_rows: list, device, path: Path,
                      job_id: str, n: int = EVAL_N_SHOW) -> None:
    rng   = np.random.default_rng(0)
    picks = sorted(rng.choice(len(eval_rows), size=min(n, len(eval_rows)), replace=False))
    sel   = [eval_rows[i] for i in picks]
    pipeline.model.model.eval()
    preds = predict_fourier(pipeline, [r["context"] for r in sel], device)
    pipeline.model.model.train()

    seg     = HORIZON
    ctx_x   = np.arange(CONTEXT_LEN)
    parts_x = [np.arange(CONTEXT_LEN + i*seg, CONTEXT_LEN + (i+1)*seg) for i in range(N_PARTS)]

    fig, axes_raw = plt.subplots(n, 1, figsize=(18, 3*n))
    axes = np.asarray(axes_raw)
    fig.suptitle(
        f"Fourier SarSim0  ·  job {job_id}\n"
        "crimson=F1(GT)  darkorange=F2(GT)  seagreen=F3(GT)  black-dash=Prediction",
    ...
    for idx, (row, pred) in enumerate(zip(sel, preds)):
        ...
        for i in range(N_PARTS):
            ...
            ax.plot(parts_x[i], tgt_i,  color=FOURIER_CFG["part_cols"][i], lw=1.8,
                    label=FOURIER_CFG["part_names"][i])
            ax.plot(parts_x[i], pred_i, color=_PRED_COL, lw=1.2, ls="--", alpha=0.8)
...
def main():
    ...
    FIGURES_DIR = FINETUNING_ROOT / "figures" / "finetunes" / f"finetune_sarsim0_fourier_{job_id}"
    CKPT_DIR    = FINETUNING_ROOT / "checkpoints" / f"finetune-sarsim0-fourier-{job_id}"
    ...
    if IS_MAIN:
        print("=" * 60)
        print("Fourier fine-tuning: chronos-t5-small  (SarSim0)")
        print(f"  Job ID  : {job_id}")
        print(f"  Device  : {device}  |  DDP={IS_DDP}  world={WORLD_SIZE}")
        print(f"  Steps   : {N_STEPS:,}  |  batch={BATCH_SIZE}  |  LR={LR}")
        print(f"  Output  : Fourier-1({HORIZON}) | SEP | Fourier-2({HORIZON}) | SEP | Fourier-3({HORIZON})")
        print("=" * 60)
    ...
    train_tok = pretokenise(train_rows, pipeline.tokenizer)

    per_rank_batch = max(1, BATCH_SIZE // WORLD_SIZE)
    if IS_DDP:
        shard = [train_tok[i] for i in range(len(train_tok)) if i % WORLD_SIZE == LOCAL_RANK]
    else:
        shard = train_tok

    loader = CyclicLoader(shard, per_rank_batch, seed=SEED)
    ...
    if IS_MAIN:
        print("\nTraining...")
    pbar = tqdm(range(1, N_STEPS+1), total=N_STEPS, desc="Training (Fourier SarSim0)",
    ...
    for step in pbar:
        ...
        if step % EVAL_EVERY == 0:
            ...
            if IS_MAIN:
                ...
                mae = evaluate(pipeline, eval_rows, device)
                eval_steps.append(step); fourier_maes.append(mae)
                tqdm.write(f"  [eval] step={step:,}  Fourier MAE={mae:.4f}")

                if mae < best_mae - EARLY_STOP_MIN_DELTA:
                    best_mae = mae; no_improve_cnt = 0
                else:
                    no_improve_cnt += 1
                    tqdm.write(
    ...
    if IS_MAIN and stopped_early:
        print(f"\nEarly stop at step {step:,}  (best Fourier MAE={best_mae:.4f})")
    ...
    if IS_MAIN:
        ...
        for name, arr in [("loss_steps", loss_steps), ("losses", losses),
                          ("eval_steps", eval_steps), ("fourier_maes", fourier_maes)]:
            np.save(str(CKPT_DIR / f"{name}.npy"), np.array(arr))

        print("\nGenerating plots...")
        inner.eval(); pipeline.model.model = inner
        plot_loss(loss_steps, losses, eval_steps, fourier_maes,
                  path=FIGURES_DIR / "loss.png", job_id=job_id)
        plot_eval_samples(pipeline, eval_rows, device,
                          path=FIGURES_DIR / "eval_fourier_samples.png", job_id=job_id)

        print(f"\nDone.  Figures → {FIGURES_DIR}/")
        if fourier_maes:
            print(f"Final Fourier MAE : {fourier_maes[-1]:.4f}")
            print(f"Best  Fourier MAE : {best_mae:.4f}")
