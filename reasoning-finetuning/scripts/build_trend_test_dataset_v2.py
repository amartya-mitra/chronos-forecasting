#!/usr/bin/env python3
"""
Build sarsim0-trend-test-v2.arrow: a testing dataset from sarsim0-10k.arrow.

Split:
  70%  fast-mode  — input context  →  original 64-step forecast (unchanged)
  30%  trend-mode — input context  →  last 64 steps of the extracted trend
...
Outputs:
  data/sarsim0-trend-test-v2.arrow
  figures/dataset_trend_test_v2/trend_test_fast_samples.png   (5 fast-mode)
  figures/dataset_trend_test_v2/trend_test_trend_mode_samples.png  (5 trend-mode)
"""
...
FAST_FRACTION     = 0.70    # 70% fast-mode, 30% trend-mode
SEED              = 42
...
def plot_fast_samples(dataset: list[dict], path: Path, n: int = 5) -> None:
    ...
    fig.suptitle(
        "Trend-Test-V2 Dataset  ·  Fast-Mode Samples (70%)\n"
...
def plot_trend_samples(dataset: list[dict], path: Path, n: int = 5) -> None:
    ...
    axes = np.asarray(axes_raw)
    fig.suptitle(
        "Trend-Test-V2 Dataset  ·  Trend-Mode Samples (30%)\n"
...
def verify_dataset(dataset: list[dict]) -> None:
    """Verify mode tags and target sizes across all datapoints."""
    errors = []
    for d in dataset:
        if d["mode"] not in ("fast", "trend"):
            errors.append(f"  {d['item_id']}: unknown mode '{d['mode']}'")
        if len(d["target"]) != PREDICTION_LENGTH:
            errors.append(f"  {d['item_id']}: target length {len(d['target'])} != {PREDICTION_LENGTH}")
    if errors:
        print(f"  ERRORS ({len(errors)}):")
        for e in errors[:10]:
            print(e)
    else:
        print(f"  All {len(dataset):,} rows pass: valid mode tag + target length = {PREDICTION_LENGTH}")
...
def main() -> None:
    print("=" * 60)
    print("Building SarSim0 Trend-Test-V2 Dataset")
    ...
    print("\nBuilding trend-test-v2 dataset...")
