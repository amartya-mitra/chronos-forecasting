#!/usr/bin/env python3
"""
Build sarsim0-fourier.arrow from sarsim0-10k.arrow.

Uses ALL 10,000 rows (fast + reasoning) — Fourier computation depends only
on the input context, not on oracle trend/seasonal labels.

Mode tag: "FOURIER"

For each row:
  1. De-mean the 512-step context.
  ...
  4. For each frequency k, reconstruct the sinusoid at the FORWARD-LOOKING
     horizon window (time steps 512 … 575):
       A  = 2 * |X[k]| / N
       φ  = angle(X[k])
       f  = k / N   (cycles per sample)
       fourier_i[j] = A · cos(2π · f · (N + j) + φ),  j = 0 … 63

  5. Per-component scale normalisation (same as ST dataset):
       s_ctx        = mean|context|
       fourier_norm = fourier * (s_ctx / max(mean|fourier|, ε))
     so that after Chronos tokeniser divides by s_ctx, each segment is O(1).

Output target: [fourier_1_norm(64) | fourier_2_norm(64) | fourier_3_norm(64)]
               = 192 steps total, ordered highest→lowest dominant frequency.
...
Outputs:
  data/sarsim0-fourier.arrow
  figures/datasets/dataset_sarsim0_fourier/samples.png   (5 sample plots)
"""
...
OUTPUT_PATH = FINETUNING_ROOT / "data" / "sarsim0-fourier.arrow"
FIGURES_DIR = FINETUNING_ROOT / "figures" / "datasets" / "dataset_sarsim0_fourier"
...
def build_dataset(rows: list[dict]) -> list[dict]:
    ...
    for r in rows:
        ctx     = r["context"]
        comps   = top3_fourier_forward(ctx)                # 3 × (64,)
        ...
        out.append({
            ...
            "mode":     "FOURIER",
            "context":  ctx,
            "target":   np.concatenate(norms),   # (192,)
            "s_first":  scales[0],
            "s_second": scales[1],
            "s_third":  scales[2],
        })
...
def plot_samples(dataset: list[dict], path: Path, n: int = 5) -> None:
    ...
    fig.suptitle(
        "SarSim0 Fourier Dataset  ·  Sample Plots  (targets rescaled to context-scale magnitude)\n"
        "[FOURIER-prefix  |  context(512)  →  fourier_1(64) | fourier_2(64) | fourier_3(64)]",
