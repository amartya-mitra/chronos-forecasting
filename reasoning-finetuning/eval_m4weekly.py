#!/usr/bin/env python3
"""
eval_m4weekly.py — Sweep evaluation: M4 Weekly prefix vs zero-shot.

2-column comparison on held-out val series (80/20 split, seed=42):
  Column 1: zero-shot        (no prefix)
  Column 2: w/m4weekly-pfx  (checkpoint: sweep-m4weekly-500steps.pt, ceiling=0.80)

Outputs:
  results/sweep_m4weekly_eval.json
  results/sweep_m4weekly_summary.txt
...
CKPT_PATH     = FINETUNING_ROOT / "checkpoints" / "sweep-m4weekly-500steps.pt"
OUTPUT_JSON   = Path("results/sweep_m4weekly_eval.json")
OUTPUT_TXT    = Path("results/sweep_m4weekly_summary.txt")
SWEEP_MD      = Path("results/sweep_summary.md")

TRAIN_SEED    = 42
NUM_SAMPLES   = 20
PRED_LEN      = 13      # M4 weekly standard forecast horizon
CONTEXT_LEN   = 80      # all 359 series; cycles=1.54 (< 2 — flagged, n_cycles=1 used)
TARGET_OFFSET = 0       # future = target[:] — raw future only
MASE_PERIOD   = 52      # M4 Weekly standard: annual seasonal naive
BATCH_SIZE    = 8
MIN_PERIOD    = 4
MAX_PERIOD    = 80      # n_cycles=1: 80 // 1 = 80 (covers period=52)
RS_THRESHOLD  = 0.4

CEIL_STEP     = 500
CEIL_TOTAL    = 500

# Audit note: context_len=80 chosen so all 359 series are retained;
# 18.1% of series are shorter than 117 pts (context+pred for 104-pt window).
# cycles_available = 80/52 = 1.54 — STL borderline; n_cycles=1, max_period=80.
...
print(f"  cycles_available = {cycles_avail:.2f}  "
      f"({'✓ >= 2.0: reliable' if cycles_avail >= 2.0 else '⚠ < 2.0: borderline'})  "
...
def run_prefix_pass(pg: PrefixGenerator):
    ...
    print("=" * 70)
    print(f"PREFIX PASS — w/m4weekly-pfx  (ceiling={ceil_val:.2f} fixed)  ({n_val} series)")
    print("=" * 70)
    ...
    print(f"\n  w/m4weekly-pfx: MSE={nanmean(mse_arr):.4f}  "
          f"MAE={nanmean(mae_arr):.4f}  MASE={nanmean(mase_arr):.4f}")
...
P("=" * 90)
P("SWEEP — M4 Weekly  (Phase 2 dataset sweep)")
...
P(f"  STL params:  min_period={MIN_PERIOD}, max_period={MAX_PERIOD}  (n_cycles=1)")
P(f"  cycles_available = {cycles_avail:.2f}  "
...
P(f"  w/m4weekly-pfx:  fired={ceil_fire*100:.2f}%  pre_ceil_mean={pre_mean:.4f}")
P(f"  (Healthy range: pre_ceil_mean ~0.40–0.70; should match training value)")

P()
P("─" * 90)
P("5. PER-SERIES IMPROVEMENT RATE (MAE)")
P(f"  w/m4weekly-pfx: {improv}/{n_val} improved  ({improv/n_val*100:.1f}%)")

P()
P("─" * 90)
P("VERDICT")
helps = agg_mase_p < agg_mase_b
P(f"  w/m4weekly-pfx vs zero-shot: {'HELPS' if helps else 'HURTS'}  "
  f"(ΔMASE={dpct(agg_mase_b, agg_mase_p):+.2f}%)")
if helps:
    P("  → In-domain training SUCCEEDED on M4 Weekly")
else:
...
try:
    ...
    updated  = existing.replace(
        ...
        f"| M4 Weekly | {CONTEXT_LEN} | {MASE_PERIOD} | {n_train} | "
        f"{agg_mase_b:.4f} | {agg_mase_p:.4f} | {dmase_str} | {improv_str} |"
    )
    updated = updated.replace(
        "_Updated after M4 Daily run._",
