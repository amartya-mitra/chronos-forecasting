#!/usr/bin/env python3
"""
eval_m4daily.py — Sweep evaluation: M4 Daily prefix vs zero-shot.

2-column comparison on 846 held-out val series (80/20 split, seed=42):
  Column 1: zero-shot        (no prefix)
  Column 2: w/m4daily-pfx   (checkpoint: sweep-m4daily-2000steps.pt, ceiling=0.80)

Outputs:
  results/sweep_m4daily_eval.json
  results/sweep_m4daily_summary.txt
...
CKPT_PATH     = FINETUNING_ROOT / "checkpoints" / "sweep-m4daily-2000steps.pt"
OUTPUT_JSON   = Path("results/sweep_m4daily_eval.json")
OUTPUT_TXT    = Path("results/sweep_m4daily_summary.txt")
...
MASE_PERIOD   = 7       # weekly seasonal naive for MASE
BATCH_SIZE    = 16
MIN_PERIOD    = 4
MAX_PERIOD    = 31      # 93 // 3 = 31 (n_cycles=3); dominant periods are 12-23
RS_THRESHOLD  = 0.4
...
def run_prefix_pass(pg: PrefixGenerator):
    ...
    print("=" * 70)
    print(f"PREFIX PASS — w/m4daily-pfx  (ceiling={ceil_val:.2f} fixed)  ({n_val} series)")
    print("=" * 70)
    ...
    print(f"\n  w/m4daily-pfx: MSE={nanmean(mse_arr):.4f}  "
          f"MAE={nanmean(mae_arr):.4f}  MASE={nanmean(mase_arr):.4f}")
...
P("=" * 90)
P("SWEEP — M4 Daily  (Phase 2 dataset sweep)")
...
P()
P("1. MAIN COMPARISON TABLE  (on 846 held-out val series)")
P()
...
P(f"  w/m4daily-pfx:  fired={ceil_fire*100:.2f}%  pre_ceil_mean={pre_mean:.4f}")
P(f"  (Healthy range: pre_ceil_mean ~0.40–0.70; should match training value)")

P()
P("─" * 90)
P("5. PER-SERIES IMPROVEMENT RATE (MAE)")
P(f"  w/m4daily-pfx: {improv}/{n_val} improved  ({improv/n_val*100:.1f}%)")

P()
P("─" * 90)
P("VERDICT")
helps = agg_mase_p < agg_mase_b
P(f"  w/m4daily-pfx vs zero-shot: {'HELPS' if helps else 'HURTS'}  "
  f"(ΔMASE={dpct(agg_mase_b, agg_mase_p):+.2f}%)")
if helps:
    P("  → In-domain training SUCCEEDED on M4 Daily")
else:
...
# Read existing sweep_summary.md to preserve prior rows
try:
    existing = SWEEP_MD.read_text()
    # Replace the M4 Daily TBD row with actual results
    updated = existing.replace(
        "| M4 Daily | — | 7 | — | TBD | TBD | TBD | TBD |",
        f"| M4 Daily | 93 | 7 | 3,381 | {agg_mase_b:.4f} | {agg_mase_p:.4f} | {dmase_str} | {improv_str} |"
    )
    ...
    # Fallback: write from scratch with all known results
    sweep_table = f"""\
...
| M4 Monthly | 36 | 12 | 8,000 | 1.2105 | 0.7274 | −39.9% | 86.8% |
| M4 Daily | 93 | 7 | 3,381 | {agg_mase_b:.4f} | {agg_mase_p:.4f} | {dmase_str} | {improv_str} |
